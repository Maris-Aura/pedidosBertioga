...
export function ThermalReceipt({
  ...
  return (
    <div
      ...
      className="hidden print:block font-mono text-xs p-2"
      style={{ width, maxWidth: width }}
    >
      <div className="text-center border-b border-black pb-2 mb-2">
        <div className={`font-bold uppercase ${compact ? "text-xs" : "text-sm"}`}>
          {store.name}
        </div>
        ...
        {order.items.map((item) => (
          <div key={item.id}>
            ...
            {item.options_selected_json.length > 0 ? (
              <div className={compact ? "text-[9px] pl-2" : "text-[10px] pl-2"}>
